...
export default function ConversationList({ conversations, activeId, onSelect, onStartChat }) {
  ...
  return (
    ...
    <input
      ...
      className="w-full rounded-full border border-hairline bg-ember-deep px-4 py-2.5 font-body text-sm text-bone placeholder-mauve outline-none transition focus:border-signal" />
  </div><ul className="flex-1 overflow-y-auto px-2 pb-4">
        ...
        return (
          <li key={c.id}>
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition ${active ? "bg-ember-deep" : "hover:bg-ember-deep/60"}`}
            >
